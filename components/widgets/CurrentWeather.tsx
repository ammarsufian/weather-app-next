...
import { convertIntoCeluis } from "@/lib/utils"
  ...
  return (
    <Card className="relative flex h-fit w-full shrink-0 flex-col justify-between overflow-hidden md:h-[25rem]">
      ...
      <div className="flex justify-center py-7 text-8xl font-bold md:py-10">
        {convertIntoCeluis(Math.round(data.main.temp))}&deg;
      </div>
      <div>
        <IconComponent
        ...
        <div className="flex gap-2 dark:text-neutral-500">
          <span>H: {convertIntoCeluis(Math.round(data.main.temp_max))}&deg;</span>
          <span>L: {convertIntoCeluis(Math.round(data.main.temp_min))}&deg;</span>
        </div>
      </div>
    </Card>
  )
